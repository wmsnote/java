...
<en-media type="image/png" hash="30ff6306fd4e28a14fe331af4bb6c009"></en-media>
...
​	那么现在`Box<Number>` n允许接受什么类型的参数？我们是否能够传入`Box<Integer>`或者`Box<Double>`呢？答案是否定的，虽然Integer和Double是Number的子类，但是在泛型中`Box<Integer>`或者`Box<Double>`与`Box<Number>`之间并没有任何的关系。这一点非常重要，接下来我们通过一个完整的例子来加深一下理解。
...
### **问题四**(不能对泛型代码直接使用instanceof关键字)

我们无法对泛型代码直接使用instanceof关键字，因为Java编译器在生成代码的时候会擦除所有相关泛型的类型信息，正如我们上面验证过的JVM在运行时期无法识别出`ArrayList<Integer>`和`ArrayList<String>`的之间的区别：
